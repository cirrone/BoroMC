...
void SDCPhysicsList::ConstructParticle()
{
...
void SDCPhysicsList::ConstructProcess()
{
    AddTransportation();
...
void SDCPhysicsList::AddPhysicsList(const G4String& name)
{
    if (verboseLevel>0) {
...
void SDCPhysicsList::SetBuilderList0(G4bool flagHP)
{
...
void SDCPhysicsList::SetBuilderList1(G4bool flagHP)
{
    fHadronPhys.push_back( new G4EmExtraPhysics(verboseLevel));
    if(flagHP) {
        fHadronPhys.push_back( new G4HadronElasticPhysicsHP(verboseLevel) );
...
void SDCPhysicsList::SetBuilderList2()
{
    fHadronPhys.push_back( new G4EmExtraPhysics(verboseLevel));
...
void SDCPhysicsList::SetCutForGamma(G4double cut)
{
    fCutForGamma = cut;
...
void SDCPhysicsList::SetCutForElectron(G4double cut)
{
    fCutForElectron = cut;
...
void SDCPhysicsList::SetCutForPositron(G4double cut)
{
    fCutForPositron = cut;
...
void SDCPhysicsList::SetCutForProton(G4double cut)
{
    fCutForProton = cut;
    SetParticleCuts(fCutForProton, G4Proton::Proton());
}

void SDCPhysicsList::List()
{
